...
For any same-document navigation, traversal, or replacement, the `currentchange` event will fire on `appHistory`:

```js
appHistory.addEventListener("currentchange", () => {
  // appHistory.current has changed: either to a completely new entry (with a new key),
...
The `window.appHistory` object has an event, `currentchange`, which allows the application to react to any updates to the `appHistory.current` property. This includes both navigations that change its value to a new `AppHistoryEntry`, and calls to APIs like `history.replaceState()`, `appHistory.updateCurrent()`, or intercepted `appHistory.navigate(url, { replace: true, state: newState })` calls that change its state or URL.
...
This event is mainly used for code that want to watch for navigation commits, but are separated from the part of the codebase that actually perform the navigation (since the navigating code can just use `appHistory.navigate().committed`). This is especially useful when migrating from `popstate` and `hashchange`, but it could also be used by e.g. analytics packages that don't care about the navigation finishing, just committing. It can also be used to set up relevant [per-entry events](#per-entry-events):
...
The event comes with a property, `from`, which is the previous value of `appHistory.current`. It also has a `navigationType` property, which is either `"reload"`, `"push"`, `"replace"`, `"traverse"`, or `null`. There are a few interesting cases to consider:

- The entry can be the same as before, i.e. `appHistory.current === event.from`. This happens when `appHistory.updateCurrent()` is called, or when `appHistory.reload()` is converted into a same-document reload. Note that in this case the old state cannot be retrieved, i.e. `event.from.getState()` will return the current app history state.

- `event.navigationType` will be `null` when `appHistory.updateCurrent()` is called, since that is not a navigation (despite updating `appHistory.current`).

- During traversals, i.e. when `event.navigationType` is `"traverse"`, you can get the delta by using `appHistory.current.index - event.from.index`. (Note that this can return `NaN` in `"replace"` cases where `event.from.index` is `null`.)
...
- To react to navigations that have finished, including any asynchronous work, use the `navigatesuccess` or `navigateerror` events on `appHistory`. Note that these will only be fired after any promises passed to the `navigate` event's `event.transitionWhile()` method have settled.

- To react to navigations that have committed (but not necessarily yet finished), use the [`currentchange` event](#current-entry-change-monitoring) on `appHistory`. This is the most direct counterpart to `popstate` and `hashchange`, so might be easiest to use as part of an initial migration while your app is adapting to a `navigate` event-centric paradigm.
